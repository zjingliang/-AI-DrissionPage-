                ...
                while True:
                    ...
                    if chaoxing.ele("xpath=//*[@id=\"video\"]/div[*]/button/span[2]", timeout=0.1):
                        if chaoxing.ele("xpath=//*[@id=\"video\"]/div[*]/button/span[2]", timeout=0.1).text == "播放":
                            chaoxing.ele("xpath=//*[@id=\"video\"]/div[*]/button/span[2]").click()
                        if chaoxing.ele("@class=tkTopic_title", timeout=0.1):
                            topic = chaoxing.ele("@class=tkTopic_title", timeout=0.5).text
                            title = chaoxing.ele("@class=tkItem_title", timeout=0.5).text
                            items = chaoxing.eles("@class=ans-videoquiz-opt", timeout=0.5)
                            title_prompts = topic + ":\n" + title
                            # color_print(topic, title)
                            for item in items:
                                title_prompts += "\n"
                                title_prompts += item.text
                                # color_print(item.text)
                            color_print(title_prompts)
                            chaoxing.ele(
                            ...
                            try:
                                for answer in answers_num:
                                    chaoxing.ele(f"xpath=//div/ul[@class=\"tkItem_ul\"]/li[{answer}]/label/span",
                                                 timeout=0.5).click()
